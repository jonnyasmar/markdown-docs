...
export function normalizeSelection(selectedText: string): NormalizedSelection {
  ...
  patterns.forEach(pattern => {
    ...
    while ((match = regex.exec(selectedText)) !== null) {
      ...
      const innerText = pattern.replacement ? match[0].replace(regex, pattern.replacement) : match[pattern.syntaxLength > 0 ? 2 : 1];
...
export function findTextInMarkdown(markdown: string, targetText: string): number {
  ...
  let index = markdown.indexOf(targetText);
  if (index !== -1) {
 ...
 */
function isFormattedMatch(markdown: string, match: { index: number; match: string }, normalized: NormalizedSelection): boolean {
 ...
 */
export function escapeDirectiveContent(text: string, preserveFormatting: boolean = false): string {
  const replacements = [
  ...
  return replacements.reduce((acc, replacement) => 
    acc.replace(replacement.regex, replacement.replacement), text
  );
}
...
export function isWithinCodeBlock(markdown: string, selectionStart: number): boolean {
  ...
  const afterSelection = markdown.substring(selectionStart);
  
  // Count code block markers before and after selection
  const codeBlocksBefore = (beforeSelection.match(/```/g) || []).length;
  const codeBlocksAfter = (afterSelection.match(/```/g) || []).length;
 ...
 */
export function analyzeSelectionContext(markdown: string, selectedText: string): {
